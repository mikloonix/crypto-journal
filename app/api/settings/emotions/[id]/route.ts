...
export async function PATCH(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    const result = await catalogSettingsService.patchEmotion(userId, id, body)
    if (!result.ok) {
    ...
    console.error(e)
    return jsonErr("PATCH emotion error", 500)
  }
}
...
export async function DELETE(req: NextRequest, ctx: Ctx) {
  ...
  try {
    const result = await catalogSettingsService.deleteEmotion(userId, id)
    if (!result.ok) {
      return jsonErr(result.error, result.status)
    }
    ...
    console.error(e)
    return jsonErr("DELETE emotion error", 500)
  }
}
