...
- ✅ Единый layout с навигацией по вкладкам (**desktop + mobile**): шапка с токенами из `UI-GUIDELINES.md` (`globals.css`), горизонтальные ссылки с **`md`** breakpoint и ниже — выезжающее меню по бургеру (`AppHeader`).
...
- **Форма открытия сделки** (`TradeOpenForm` на `/dashboard`): адаптивная сетка — одна колонка на узком экране, далее `sm` / `md` / `lg` / `xl`, чтобы поля не сжимались в два узких столбца на телефоне.
